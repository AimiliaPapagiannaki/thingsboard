...
def main(argv):
    ...
    if reportID == 'b':
        ...
        workbook = Workbook(filename, {'constant_memory': True,'nan_inf_to_errors':True})
        for device in r1:
            ...
            #     workbook.add_worksheet().write('Summary', sum_nrg)
            ...
            if summary.empty == False:
                smr = summary.to_dict(orient = 'index')
                devName = devName.replace(':', '')
                worksheet = workbook.add_worksheet(devName)

                i=0

                for value in smr.values():
                    j = 0
                    for value2 in value.values():
                        worksheet.write(i,j,value2)
                        j = j + 1  # column index
                    i = i + 1  # row index
